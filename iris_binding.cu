...
extern __global__ void preprocess_kernel(const uint32_t *__restrict__ data,
                                         const uint32_t *__restrict__ mask,
                                         uint32_t *__restrict__ premasked,
                                         int M);

extern __global__ void min_hamming_kernel(
    const uint32_t *__restrict__ premasked, const uint32_t *__restrict__ mask,
    int M, float *__restrict__ D, bool write_output, bool collect_pairs,
    float threshold, uint2 *__restrict__ pairs, unsigned int *match_count,
...
masked_hamming_cuda(torch::Tensor data, torch::Tensor mask, bool write_output,
  ...
  // Grid/block/smem match iris.cu (defaults)
